...
import {
  FaReact, FaNodeJs, FaPython, FaShieldAlt,
  FaMicrochip, FaKey, FaGitAlt, FaFlask,
} from 'react-icons/fa'
import { useIsMobile } from '../../hooks/useIsMobile'

type SkillLevel = 'Production' | 'Research Grade' | 'Research' | 'Shipped' | 'Daily Use' | 'Published'

const skills: { Icon: React.ElementType; name: string; items: string; level: SkillLevel; pct: number }[] = [
  { Icon: FaReact,      name: 'Frontend',    items: 'React.js · HTML5 · CSS3\nJavaScript ES6+',                   level: 'Production',    pct: 88 },
  { Icon: FaNodeJs,     name: 'Backend',     items: 'Node.js · Express\nMongoDB · SQL · REST',                     level: 'Production',    pct: 85 },
  { Icon: FaPython,     name: 'ML / AI',     items: 'Python · TensorFlow\nPyTorch · Scikit-learn\nGemini API',      level: 'Research Grade', pct: 78 },
  { Icon: FaShieldAlt,  name: 'Security',    items: 'Wireshark\nIntrusion Detection\nNetwork Analysis',             level: 'Research',      pct: 70 },
  { Icon: FaMicrochip,  name: 'IoT',         items: 'ESP32 · C\nSensor Networks\nEmbedded Systems',                 level: 'Shipped',       pct: 72 },
  { Icon: FaKey,        name: 'Auth & APIs', items: 'JWT · OCR\nOAuth · REST APIs\nMVC Architecture',               level: 'Production',    pct: 83 },
  { Icon: FaGitAlt,     name: 'DevOps',      items: 'Git · GitHub\nVercel · Render\nLinux · Postman',               level: 'Daily Use',     pct: 80 },
  { Icon: FaFlask,      name: 'Research',    items: 'Academic Writing\nData Analysis\nElsevier ICT Express',        level: 'Published',     pct: 82 },
]
    ...
    <section style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '1.6rem', flexWrap: 'wrap', gap: '0.5rem' }}>
        <div>
          ...
          <div style={{ fontFamily: 'JetBrains Mono,monospace', fontSize: '0.65rem', letterSpacing: '0.14em', textTransform: 'uppercase', color: 'rgba(75,191,255,0.5)', marginTop: '0.5rem' }}>
        ...
        {skills.map((s, i) => (
          <motion.div key={i}
            ...
            style={{
              ...
              minHeight: isMobile ? 120 : 140, position: 'relative',
          ...
          >
            ...
            <div style={{ fontFamily: 'Inter,sans-serif', fontWeight: 700, fontSize: isMobile ? '0.82rem' : '0.9rem', color: 'var(--cream)' }}>{s.name}</div>
            <div style={{ fontFamily: 'JetBrains Mono,monospace', fontSize: '0.65rem', color: 'var(--muted)', lineHeight: 1.8, whiteSpace: 'pre-line', flex: 1 }}>{s.items}</div>

            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem', marginTop: 'auto' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontFamily: 'JetBrains Mono,monospace', fontSize: '0.6rem', letterSpacing: '0.14em', textTransform: 'uppercase', color: levelColors[s.level] }}>{s.level}</span>
                <span style={{ fontFamily: 'JetBrains Mono,monospace', fontSize: '0.6rem', color: 'rgba(255,255,255,0.4)' }}>{s.pct}%</span>
              </div>
